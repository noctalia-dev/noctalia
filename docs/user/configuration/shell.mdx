...
border_color          = "outline"   # floating panel outline color: theme role token or #RRGGBB
border_width          = 1.0         # floating panel outline width in px, 0 hides the stroke
...
- `shell.panel.border_color` sets the floating panel outline color: a theme role token (for example `outline`, `primary`) or a fixed hex color (`#RRGGBB` / `#RRGGBBAA`). The alpha is multiplied by the panel background opacity from `shell.panel.transparency_mode`.
- `shell.panel.border_width` sets the floating panel outline width in logical pixels (default `1.0`, range `0` to `20`). Ignored when `shell.panel.borders` is `false`.
