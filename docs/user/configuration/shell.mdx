...
active_monitor_only = false # true = show popups only on the currently focused monitor
...
Set `active_monitor_only = true` to show each OSD popup on the monitor that is focused when it appears. This setting
overrides the static `monitors` list while enabled. `monitors` limits OSD popups to the listed connector names (same
matching rules as notification toasts and the dock);
