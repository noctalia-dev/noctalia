...
active_monitor_only      = false       # true = show toasts only on the currently focused monitor
...
- `active_monitor_only = true` shows each new toast on the monitor that is focused when the notification arrives. It overrides `monitors` while enabled.
