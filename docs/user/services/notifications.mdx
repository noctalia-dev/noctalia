...
- `border_color` sets the toast outline color: a theme color role name (e.g. `primary`) or a hex value (e.g. `#a8b185`). Defaults to the theme `outline` role and applies to normal and low urgency toasts; critical toasts keep their red urgency outline. Configure in **Settings → Notifications → Toasts**.
- `border_width` sets the toast outline stroke width in px, 0 to 6 in 0.5 steps; 0 hides the outline. Defaults to 1 px and applies to normal and low urgency toasts; critical toasts keep the emphasized 3 px width. Configure in **Settings → Notifications → Toasts**.
