...
| `layer` | `<entry>_layer` | `follow` (default), `top` or `overlay`; only applies when floating (optional manifest default requires <PluginApiBadge feature="panel-layer" />) |
...
**Layer** controls the floating surface's layer-shell layer. `follow` uses the shell's floating panel layer,
`top` keeps the panel under fullscreen windows; `overlay` keeps it above them, which is what an on-screen keyboard
or heads-up panel needs. An attached panel ignores this setting and sits at its host bar's configured layer.
