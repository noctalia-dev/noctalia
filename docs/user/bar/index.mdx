...
Bars are defined as named subtables under `[bar.*]`. Each bar is spawned on every connected monitor (or only those listed in its `monitors` allowlist), then per-monitor overrides are applied.
...
enabled            = true
monitors           = []          # connector names to show this bar on; empty = all monitors
...
`monitors` is an allowlist of connector names (or description substrings, matched like a per-monitor `match`) where this bar appears. Leave it empty (the default) to show the bar on every connected monitor. In the Settings GUI it lives under **Bar → General → Monitors**, with a dropdown of detected outputs.

`monitors` and per-monitor overrides are independent gates that compose:

- With `monitors` empty, nothing is filtered and overrides behave exactly as before.
- A monitor **not** in `monitors` never shows the bar, even if an override sets `enabled = true` for it (the allowlist is applied last, so it always wins for exclusion).
- A monitor **in** `monitors` still obeys its override, so an override can disable or restyle the bar there.

Use `monitors` for the simple "which monitors" choice, and per-monitor overrides for deeper per-monitor tweaks.
