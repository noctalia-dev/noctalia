...
Hovering a named workspace shows its full name in a tooltip whenever the pill does not already spell it out: when `max_label_chars` cut it short, when `label_source = "id"` shows a number instead, or when labels are off.
