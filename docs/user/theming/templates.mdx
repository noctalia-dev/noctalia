...
When Noctalia exits, hooks that are still running get 5 seconds to finish. After that their process group is
terminated, so a hook that never exits cannot keep Noctalia from quitting. `output_path_dynamic` commands follow the
same rule. A hook that rewrites a file should write a temporary file and move it into place, so an interrupted run
