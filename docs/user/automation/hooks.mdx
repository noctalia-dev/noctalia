...
| `lockscreen_wallpaper_changed` | After the effective lock screen wallpaper changes: an explicit `[lockscreen].wallpaper` override being set or cleared, or the desktop wallpaper changing while no override is set. Sets `NOCTALIA_LOCKSCREEN_WALLPAPER_PATH`. |
