...
| `network` | Network | Wi-Fi, cellular, connection status, saved profiles, and NetworkManager-backed actions |
...
The Network tab uses the configured network backend to show Wi-Fi, wired, VPN, and cellular connection details. With NetworkManager, it can toggle mobile broadband, activate or remove saved cellular profiles, create profiles from a name and APN, and prompt for a SIM PIN.

The current connection line lists the IPv4 address, signal strength, radio details, and external IP. Wi-Fi shows its band (`2.4 GHz`, `5 GHz`, `6 GHz`, or `60 GHz`) when the backend reports the connection frequency. Cellular connections show their access technology, such as LTE. Cellular controls require NetworkManager and supported modem hardware.
